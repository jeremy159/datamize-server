use datamize_domain::{
    BudgetDetails, BudgeterConfig, DatamizeScheduledTransaction, ExpenseCategorization, MonthTarget,
};
use fake::{Fake, Faker};
...
use crate::services::budget_template::tests::template_detail::testutils::TestContext;
...
async fn returns_success_and_default_struct_when_nothing_in_db(pool: SqlitePool) {
    check_get(
        pool,
        MonthTarget::default(),
