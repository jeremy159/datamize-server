...
async fn check_get_by_category_id(
    pool: SqlitePool,
    ...
    let response = service.get_transactions_by_category_id(category_id).await;
...
async fn returns_empty_list_when_nothing_in_db(pool: SqlitePool) {
    check_get_by_category_id(pool, &[], Faker.fake(), Some(vec![]), None).await;
}

#[sqlx::test(migrations = "../db-sqlite/migrations")]
async fn returns_empty_list_when_nothing_linked(pool: SqlitePool) {
    check_get_by_category_id(
        pool,
